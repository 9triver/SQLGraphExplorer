...
string_function
    : SUBSTR '(' expression ',' expression (',' expression)? ')'    #substr_function
    | TO_CHAR '(' (table_element | standard_function | expression)
                  (',' quoted_string)? (',' quoted_string)? ')'     #to_char_function
    | DECODE '(' expressions  ')'                                   #decode_function
    | CHR '(' concatenation USING NCHAR_CS ')'                      #chr_function
    | NVL '(' expression ',' expression ')'                         #nvl_function
    | TRIM '(' ((LEADING | TRAILING | BOTH)? expression? FROM)? concatenation ')'                   #trim_function
    | TO_DATE '(' (table_element | standard_function | expression)
       (DEFAULT concatenation ON CONVERSION ERROR)? (',' quoted_string  (',' quoted_string)? )? ')' #to_date_function
    ;
